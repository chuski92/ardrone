...
for(unsigned int ii = 0; ii < circles.size(); ii++ )
{
    if ( circles[ii][0] != -1 )
    {
            ...
            cv::circle(cv_img_out_.image, center, 10, cv::Scalar(255,0,0), -1, 8, 0 );// circle center in red
            cv::circle(cv_img_out_.image, center, radius, cv::Scalar(0,0,255), 3, 8, 0 );// circle perimeter in blue
    }
}
    ...
    // Creem un vector V de dimensions X,Y

    VectorXd v(3);
    v(0) = center.x; // Agafem ela coordenada x del centre del cercle (Bola)
    v(1) = center.y; // Agafem la coordenada y del centre del cercle (Bola)
    v(2) = 1;

    // Càlcul del vector v trasposat
    VectorXd v_T(3);
    v_T = v.transpose();

    //std::cout << "matrixK: " << std::endl << v_T << std::endl;

    // Calcul de la matriu K inversa
    Matrix3d matrixK_inv;
    matrixK_inv = matrixK_.inverse();

    // Calculem la direcció de la bola
    direction_ = matrixK_inv*v_T;

     // just to draw something with the arrow marker
...
void RosImgProcessorNode::publishMarker()
{
	...
	marker_msg.lifetime = ros::Duration(200);
